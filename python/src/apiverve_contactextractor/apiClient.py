...
class ContactextractorAPIClientError(Exception):
...
class ValidationError(ContactextractorAPIClientError):
...
class ContactextractorAPIClient:
    ...
    VALIDATION_RULES = {"url": {"type": "string", "required": True, "format": "url"}, "limit": {"type": "number", "required": True, "default": "5"}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the ContactextractorAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises ContactextractorAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise ContactextractorAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[ContactextractorAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns ContactExtractorData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises ContactextractorAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise ContactextractorAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise ContactextractorAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise ContactextractorAPIClientError(f"Request failed: {str(e)}")
